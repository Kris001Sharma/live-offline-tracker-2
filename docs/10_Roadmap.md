...
| **10A.5-D** | Trusted Device Registration Contract & Browser-Safe Device Verification Foundation | 🟡 IMPLEMENTED (contract + foundation automated-validated — engine 40/40, matrix 492/492; browser State A→B registration BLOCKED by pre-existing auth↔worker identity gap — queued wiring slice) |
| **10A.5** | Device Verification (trusted-device gate after login) | ⏳ PLANNED — blocked until the queued identity-resolution wiring slice lands |
...
**Trusted-device registration foundation (10A.5-D, implemented, NOT baseline-verified)**: The registration contract and browser-safe device-identity foundation are implemented: a device-identity provider boundary (native `ANDROID_ID` on Capacitor; deterministic per-profile browser identity marked DEVELOPMENT/TEST ONLY, with override for testing); Migration 002 enforcing one active trusted device per worker via a partial unique index (applied to live Supabase and behaviorally verified); repository `registerActive()`/`resetActive()`/`REVOKED`; registration-engine `status()` `verification` result (`NOT_REGISTERED`/`TRUSTED`/`DIFFERENT_DEVICE`/`ERROR`), `registerCurrentDevice()` via `registerActive()`, and the administrator `resetActiveDevice(workerId)` capability (domain-level only, history preserved); and the browser `DeviceVerificationGate` with the four required states (A register prompt / B trusted → Dashboard / C different device blocked / D clear error) plus Sign Out in all non-trusted states. Automated validation green (engine 40/40, full matrix 492/492). **Browser State A→B self-registration is BLOCKED** by a pre-existing auth↔worker identity gap (`AuthSession.mapToWorker()` sets the auth UUID as a placeholder and the local `workers` table is empty, so the `trusted_devices.worker_id` foreign key cannot be satisfied). Classification: pre-existing wiring gap, not a 10A.5-D defect; **queued for a dedicated wiring slice** (resolve canonical worker identity for the authenticated session so `currentWorker.id` matches a canonical local `workers` row). 10A.5-D is **NOT BASELINE VERIFIED** — UI closure deferred until that wiring slice lands.

**Next roadmap capability**: **10A.5-W (queued wiring slice) — resolve canonical worker identity for an authenticated session** so browser State A→B trusted-device registration completes; then finish 10A.5-D browser validation and close it as BASELINE VERIFIED, unblocking the full **10A.5 — Device Verification** gate, the Worker Journey step immediately following Worker Login (docs/12_Product_Design.md): the app confirms the physical device matches the worker's registered trusted device before granting dashboard access, consuming the `TrustedDeviceRegistrationEngine` public API (`status()` / `registerCurrentDevice()` / result codes).
