...
**Native Android device identity activation (10A.5-N, implemented, ANDROID RUNTIME VALIDATION PENDING)**: The JS-side identity chain (native provider → `TrustedDeviceEngine.load()` → `IdentityResolver` `AUTHENTICATED_DEVICE_AVAILABLE`) was already implemented and frozen (10A.5-D/W/WV), but the repository had no Capacitor Android application to run it on. 10A.5-N provided the minimum native-platform activation: `capacitor.config.ts` (`appId com.sapana.tracker`, `appName Sapana Live Tracker`, `webDir dist`), new dependencies `@capacitor/android` + `@capacitor/cli` (Capacitor 8 family, aligned with the existing plugins; no existing dependency range changed — the dependency decision was surfaced to and approved by the repository owner before proceeding), and a generated `android/` native project via `npx cap add android` (CLI-only; standard template, `@capacitor/device` `DevicePlugin` registered; SDK `android-36`/`minSdk 24` matching the installed SDK). `gradlew :app:assembleDebug` **BUILD SUCCESSFUL**; `tsc --noEmit` PASS; vite build PASS. The browser path is untouched (browser identity remains non-authoritative, no browser registration, no DB mutation). No frozen engine, resolver, gate, router, lifecycle, schema, or migration was modified; no trusted-device record is created and no registration occurs automatically. **Android runtime verification is NOT claimed** — the recommended next slice is a native runtime validation slice (e.g. **10A.5-NV**) via the established Android/ADB/USB-debugging workflow.
